 ...
 */
export function generateRuntimeFile(options: RuntimeFileOptions): string {
  const { usage, schemaImport } = options;
  const lines: string[] = [];
  ...
  lines.push(`import type * as __schema from "${schemaImport}";`);
  lines.push("");
