...
export function generateEntryPointFile(
  ...
  // Import DrizzleDb type from runtime
  const typeImport = `import { type DrizzleDb } from "${options.runtimeImport}";`;
...
export function generateMultiFileOutput(
  ...
  // _runtime.ts - runtime helpers and DrizzleDb type
  // Schema import needs adjustment for being inside drizzleloaders/
  const runtimeSchemaImport = adjustSchemaImportPath(options.schemaImport);
  files.set(
    "drizzleloaders/_runtime.ts",
    generateRuntimeFile({
      usage: runtimeUsage,
      schemaImport: runtimeSchemaImport,
    }),
  );
