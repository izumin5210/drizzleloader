...
export function createUserRolesLoaders(db: DrizzleDb) {
  const byUserIdAndRoleId = new DataLoader<{ userId: number; roleId: number }, InferSelectModel<typeof __schema.userRoles>, string>(
    async (keys) => {
      const rows = await queryCompositeKey(db, __schema.userRoles, [__schema.userRoles.userId, __schema.userRoles.roleId], ["userId", "roleId"], keys as readonly Record<string, unknown>[]);
